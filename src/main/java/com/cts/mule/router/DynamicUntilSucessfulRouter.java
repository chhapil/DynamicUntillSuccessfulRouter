...
/**
 * UntilSuccessful attempts to route a message to the message processor it
 * contains. Routing is considered successful if no exception has been raised
 * and, optionally, if the response matches an expression.
 *
 * UntilSuccessful internal route can be executed synchronously or
 * asynchronously depending on the threading profile defined on it. By default,
 * if no threading profile is defined, then it will use the default threading
 * profile configuration for the application. This means that the default
 * behavior is to process asynchronously.
 *
 ...
 * UntilSuccessful is backed by a {@link ListableObjectStore} for storing the
 * events that are pending (re)processing.
 *
 * To execute until-successful asynchronously the threading profile defined on
 * it must have doThreading attribute set with true value.
 *
...
public class DynamicUntilSucessfulRouter extends AbstractOutboundRouter implements DynamicUntilSucessfulConfig {
	...
	private String maxRetriesExpression = null;
	...
	public void initialise() throws InitialisationException {
		...
		if (synchronous) {
			this.untilSuccessfulStrategy = new SynchronousDynamicUntilSuccessfulProcessingStrategy();
		} else {
			if (threadingProfile == null) {
				threadingProfile = muleContext.getDefaultThreadingProfile();
			}
			this.untilSuccessfulStrategy = new AsyncDynamicUntillSuccessfullStrategy();
			((MessagingExceptionHandlerAware) this.untilSuccessfulStrategy)
					.setMessagingExceptionHandler(messagingExceptionHandler);
		}
	...
	protected MuleEvent route(final MuleEvent event) throws MessagingException {
		logger.info("In Dynamic Router route envent");
	...
	public String getMaxRetriesExpression() {
		return maxRetriesExpression;
	}
