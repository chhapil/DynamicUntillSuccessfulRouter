...
import static com.cts.mule.router.DynamicUntilSucessfulRouter.PROCESS_ATTEMPT_COUNT_PROPERTY_NAME;
import static com.cts.mule.router.DynamicUntilSucessfulRouter.DEFAULT_PROCESS_ATTEMPT_COUNT_PROPERTY_VALUE;
 ...
 */
public class AsyncDynamicUntillSuccessfullStrategy extends AbstractDynamicUntilSuccessfulProcessingStrategy implements Initialisable, Startable, Stoppable, MessagingExceptionHandlerAware
{
    ...
    {
    	logger.info("Async Dynamic Stategey: doRoute");
        try
        {
            ...
            logger.info("Async Dynamic Stategey: doRoute message routed");
            return processResponseThroughAckResponseExpression(event);
           
        }
        catch (final Exception e)
        {
        	 logger.info("Async Dynamic Stategey: doRoute caught exception:" +e);
    ...
    private void incrementProcessAttemptCountAndRescheduleOrRemoveFromStore(final Serializable eventStoreKey) throws Exception
    {
        try
        {
			...
			logger.info("Expression in AsyncDynamicUntilSuccessfulProcessingStrategy"
					+ expression);
            ...
            if (deliveryAttemptCount <= dynamicRetryValue)
            {
    ...
    public static Serializable buildQueueKey(final MuleEvent muleEvent)
    {
		...
		return new QueueKey(QueuePersistenceObjectStore.DEFAULT_QUEUE_STORE,
				key);
    }
